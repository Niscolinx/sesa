...
export const SITEWORKER_REPORT: ReportDetail[] = Array.from({
    length: 10,
}).map((_, i) => ({
    id: i,
...
function SiteWorkerReport() {
    ...
    useEffect(() => {
        setTimeout(() => {
            setSiteWorkerReport(SITEWORKER_REPORT)
        }, 500)
    }, [])
    ...
    const [paginate, setPaginate] = useState<Paginate>({
        ...
        totalPage: Math.ceil(siteWorkerReport.length / perPage),
        slicedPages: null,
    })
    ...
    const handleItemsPerPage = (e: ChangeEvent<HTMLSelectElement>) => {
        ...
        for (let i = 0; i < siteWorkerReport.length; i += item) {
            slicedPages.push(siteWorkerReport.slice(i, i + item))
        }
        ...
        setPaginate((prev) => {
            return {
                ...
                currentPage: 1,
                slicedPages,
                totalPage: Math.ceil(siteWorkerReport.length / item),
            }
        })
    }
    ...
    useEffect(() => {
        ...
        for (
            let i = 0;
            i < siteWorkerReport.length;
            i += paginate.itemsPerPage
        ) {
            slicedPages.push(
                siteWorkerReport.slice(i, i + paginate.itemsPerPage)
            )
        }
        ...
        setPaginate((prev) => {
            return {
                ...
                slicedPages,
                totalPage: Math.ceil(
                    siteWorkerReport.length / paginate.itemsPerPage
                ),
            }
        })
    }, [siteWorkerReport])
