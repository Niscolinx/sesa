...
const TableData = () => {
	...
	useEffect(() => {
		const fields = [...THeader].filter(
			(item) =>
				item.toLowerCase() !== "actions" && item.toLowerCase() !== "status",
		);
	...
	const handleSearch = (e: ChangeEvent<HTMLInputElement>) => {
		...
		const findData = extractedData.filter((item) => {
			return check_type(item[check_type(filterBy)]).includes(check_type(value));
		});
			...
			<div className="rounded-lg mt-[3rem]">
				{fetched.length > 0 ? (
						...
						<div className="rounded-lg mt-[3rem] ">
							<div className="grid text-[1.6rem]">
								...
								<div className="grid bg-white">
									<div
										className={`grid justify-between text-color-dark-1 bg-color-grey p-8  items-center gap-8 text-[1.4rem] capitalize grid-cols-${headerLength}`}
									>
										{THeader.slice(0, headerLength).map(
					...
					<section className="grid  place-content-center w-full h-full justify-items-center gap-4 bg-white rounded-lg">
						...
						<p className="text">Ooops you have not added any {title} yet</p>
						{!isDataProvided && (
							<button
